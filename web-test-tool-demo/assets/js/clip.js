class Clip {
  ...
  init(file, cropWidth, cropHeight) {
    this.sx = 0; //裁剪框的初始x
    this.sy = 0; //裁剪框的初始y
    this.sWidth = cropWidth || 88; //裁剪框的宽
    this.sHeight = cropHeight || 88; //裁剪框的高
    ...
    return new Promise(((resolve, reject) => {
      ...
      images.onload = function () {
        ...
        t.drawImageIOSFix(ctx, images, cropWidthScale * t.sx, cropHeightScale * t.sy,
          t.sWidth * cropWidthScale, t.sHeight * cropHeightScale, 0, 0, saveCanvas.width, saveCanvas.height);
        //    ctx.drawImage(images,2 * t.sx, 2 * t.sy, t.sWidth * 2, t.sHeight * 2, 0, 0, saveCanvas.width, saveCanvas.height);

        const dataURL = saveCanvas.toDataURL()
        if (t.attrName) {
          console.log('attrName exists')
          t.$vm[t.attrName] = dataURL;
        }
        // 保存裁剪后的blob对象
        t.cropImg = t.dataURItoBlob(dataURL)
